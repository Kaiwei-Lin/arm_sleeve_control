/*
 * pose_control_get_pvct.c
 * 功能：对电机设置位置模式，使能电机，设置电机位置值，循环获取实际快速帧pvctfast
 * @author  xufuliang
 * @date    2026-03-13
 */
#include <stdio.h>
...
int main(int argc, char* argv[]) {
     ...
     //生成主板对象
    RobotCtx* ctx = robot_create(0xFE);
    if (!ctx) return -1;
    //连接主板
    int value = robot_config_net(ctx, "192.168.3.245", 15020, 15000, "192.168.3.10");
    if (value) 
    {
        printf("连接主板成功\n");
    }
    ...
    //设置电机上传频率
    value = robot_set_fast_mode(ctx, 500);
    ...
    //生成电机对象
    RobotMotor* motor = robot_create_motor(ctx, 25, 2);


    //关闭MIT使能状态
    robot_StateMachine(ctx,0x80);

    //设置MIT使能状态
    robot_StateMachine(ctx,1);
    while (1)
    {
        
        //关闭电机心跳
        value = robot_motor_setHeartbeat(motor, 0);
    ...
    //     if(!robot_motor_getPosPID(motor,&VelP,&VelI,&VelD))
    //     {
    //          printf("motor getvelPID  failed\n");

    //     }
    //     else
    //     {
    //          printf("motor getvelPID  true\n");

    //     }


    //     if(!robot_motor_setPosPID(motor,VelP,VelI,VelD))
    //     {

    //         printf("motor SetPD  failed\n");
    //     }
    //     else
    //     {
    //         printf("motor SetPD  true\n");
    //         break;
    //     }
    // }
    ...
    while (1) 
    {
        ...
        float Pos = 0.0, Vel = 0.0,  Tor_e = 0.0;
        uint32_t FastStateMechine = 0,FastMosTemperature = 0, FastWindingTemperature = 0,
        FastBusVoltage = 0,  FastErrorCode = 0;
        //获取电机pvct值
        robot_motor_get_PVCTFast(motor, &Pos, &Vel, &Tor_e,&FastStateMechine,
        &FastMosTemperature, &FastWindingTemperature,
        &FastBusVoltage, &FastErrorCode);
      
        printf("Pos: %f, Vel: %f, Tor_e: %f, FastStateMechine: %d, FastMosTemperature: %d, FastWindingTemperature: %d, FastBusVoltage: %d, FastErrorCode: %d\n", Pos, Vel, Tor_e, FastStateMechine, FastMosTemperature, FastWindingTemperature, FastBusVoltage, FastErrorCode);
     
        robot_motor_set_position(motor,-0.45);
